...
public:
    ...
    // 获取节点在指定位置的索引
    virtual int findNodeAt(const QPointF &pos, qreal threshold = 5.0) const
    {
        Q_UNUSED(pos);
        Q_UNUSED(threshold);
        return -1;
    }
    virtual bool isPointOnPath(const QPointF &pos, qreal threshold = 5.0) const
    {
        Q_UNUSED(pos);
        Q_UNUSED(threshold);
        return false;
    ...
    // 选择边框显示控制
    void setShowSelectionIndicator(bool show)
    {
    ...
    virtual void setNodePoint(int index, const QPointF &pos)
    {
        Q_UNUSED(index);
        Q_UNUSED(pos);
    }
    ...
    // 处理节点拖动时的位置约束
    virtual QPointF constrainNodePoint(int index, const QPointF &pos) const
    {
        Q_UNUSED(index);
        return pos;
    }
